...
RUN git clone --depth=1 https://gitlab.freedesktop.org/virgl/virglrenderer.git /virglrenderer && \
    ...
    patch -p1 < /src/virgil3d/MINGW-packages/0006-virglrenderer-experimental-gl46-caps.patch && \
    patch -p1 < /src/virgil3d/MINGW-packages/0007-virglrenderer-angle-alpha-format-gles-fallback.patch && \
